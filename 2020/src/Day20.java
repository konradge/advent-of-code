...
public class Day20 implements Day {
    ...
    public String partOne() {
        long res = (long) solution[0][0].id * solution[puzzleSize - 1][0].id
                * solution[puzzleSize - 1][puzzleSize - 1].id * solution[0][puzzleSize - 1].id;
        return "" + res;
    }
    ...
    public void solvePuzzle(int i, int j, ArrayList<Tile> remainingTiles, Tile[][] puzzle) {
        ...
        for (Tile tile : remainingTiles) {
            if (isCorner && corners.stream().noneMatch(c -> c == tile))
                continue;
            if (isEdge && edges.stream().noneMatch(c -> c.id == tile.id))
                continue;
            for (Tile t : tile.getAllConfigurations()) {
                if (matches(t, i, j, puzzle)) {
                    Tile[][] clone = clonePuzzle(puzzle);
                    clone[i][j] = t;
                    solvePuzzle(i + 1, j, new ArrayList<>(
                            remainingTiles.stream().filter(remTile -> remTile.id != t.id).collect(Collectors.toList())),
                            clone);
                }
            }
        }
    ...
    public ArrayList<Tile> intersectLists(ArrayList<Tile> l1, ArrayList<Tile> l2) {
        return new ArrayList<Tile>(l1.stream()
                .filter(tile -> l2.stream().filter(t -> t.id == tile.id).collect(Collectors.toList()).size() != 0)
    ...
    public class Tile {
        ...
        public boolean hasNoMatch(Directions dir) {
            for (Tile t : allTiles.stream().filter(tile -> tile.id != this.id).collect(Collectors.toList())) {
                if (matches(this, t, dir))
                    return false;
            }
            return true;
        }
        ...
        public boolean matches(Tile t1, Tile t2, Directions dir) {
            for (Directions otherDir : Directions.getDirections()) {
                if (t1.getEdge(dir).equals(t2.getEdge(otherDir)))
                    return true;
                if (t1.getEdge(dir).equals(new StringBuilder(t2.getEdge(otherDir)).reverse().toString()))
                    return true;
            }
            return false;
        }
        ...
        /**
         * @return All possible configurations from this tile after flipping and
         *         rotating
         */
